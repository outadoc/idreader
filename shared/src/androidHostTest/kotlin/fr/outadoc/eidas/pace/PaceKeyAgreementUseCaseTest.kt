...
class PaceKeyAgreementUseCaseTest {
    ...
    private val mappedGenerator =
        EcPoint(
            x = "0F0D69FDE03A7A7DCEDD84190CCB6E466A97BFD20CA79B2B556C5A22487FFD9A".hexBytes(),
            y = "7E7B907FDD4ADAD3F57B70E3A34631EFEC15A984BD5B4589EC73C57E144BBCC0".hexBytes(),
        )
    ...
    @Test
    fun derivesCorrectSessionKeysFromChipPublicKey() = runTest {
        val step3Response =
        ...
        val useCase =
            PaceKeyAgreementUseCase(
                tagReader = StubNfcTagReader(step3Response.toByteArray()),
        ...
        val expectedKEnc = cryptoEngine.deriveKeyFromSecret(algorithm, sharedSecret, ubyteArrayOf(), 1)
        val expectedKMac = cryptoEngine.deriveKeyFromSecret(algorithm, sharedSecret, ubyteArrayOf(), 2)
    ...
    fun sharedSecretIsCommutative() {
        ...
        val secretAB = cryptoEngine.computeSharedSecret(algorithm, AndroidPrivateKey(scalarA), pubBPoint)
        val secretBA = cryptoEngine.computeSharedSecret(algorithm, AndroidPrivateKey(scalarB), pubAPoint)
...
private fun UByteArray.sliceArray(range: IntRange): UByteArray =
    toByteArray().sliceArray(range).toUByteArray()
