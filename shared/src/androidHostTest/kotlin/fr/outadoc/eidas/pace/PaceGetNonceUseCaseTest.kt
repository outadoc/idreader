...
class PaceGetNonceUseCaseTest {
    ...
    @Test
    fun decryptsChipNonceCorrectly() = runTest {
        val testCan = "123456"
        val plainNonce = ByteArray(32) { it.toByte() }

        val canBytes = testCan.toByteArray(Charsets.US_ASCII).toUByteArray()
        val kPi = cryptoEngine.deriveKeyFromSecret(algorithm, canBytes, ubyteArrayOf(), 3)
        val encryptedNonce = encryptAesCbc(kPi.toByteArray(), plainNonce)
        ...
        val useCase =
            PaceGetNonceUseCase(
                tagReader = StubNfcTagReader(mseResponse.toByteArray(), step1Response.toByteArray()),
