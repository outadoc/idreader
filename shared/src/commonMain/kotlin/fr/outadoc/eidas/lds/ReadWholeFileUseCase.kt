...
private val TAG = "ReadWholeFileUseCase"

@OptIn(ExperimentalUnsignedTypes::class)
class ReadWholeFileUseCase(
    ...
    suspend operator fun invoke(
        ...
        logger.i(TAG, "SELECT FILE $fileLabel")

        nfcSession
            .transceive(
                commandFactory.selectFile(fileId),
            ).flatMap { it.getData() }
            .getOrElse { return Result.failure(it) }

        logger.i(TAG, "READ BINARY $fileLabel offset=0")
        ...
        while (buffer.size.toInt() < totalLength) {
            ...
            logger.i(TAG, "READ BINARY $fileLabel offset=$offset remaining=$remaining")
